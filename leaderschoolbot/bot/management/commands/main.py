...
@save_user_and_messages
def spiski(update, context):
    chat = update.effective_chat
    context.bot.send_message(
        chat_id=chat.id,
        text=(
            'Со списками поступающих Вы можете ознакомиться' +
            '[по ссылке](https://priem2024.spmi.ru/) \n'
        ),
        parse_mode="MARKDOWN"
    )
...
def aspirant(update, context):
    ...
    buttons = ReplyKeyboardMarkup(
        [['Сроки подачи документов АСП', 'Документы для поступления АСП'],
         ...
         ['Конкурсные группы и количество мест АСП',
          'Способы подачи документов АСП'],
...
@save_user_and_messages
def vi_asp(update, context):
    chat = update.effective_chat
    context.bot.send_message(
        chat_id=chat.id,
        text=(
            'Поступающие в аспирантуру сдают ' +
            'следующие вступительные экзамены:\n' +
            '- Собеседование;\n' +
            '- Защита Научного задела по теме и объекту ' +
            'научных исследований.\n' +
...
@save_user_and_messages
def docs_asp(update, context):
    chat = update.effective_chat
    context.bot.send_message(
        chat_id=chat.id,
        text=(
            '1.	Заявление о приеме в аспирантуру на имя ректора ' +
            'Санкт-Петербургского горного университета императрицы  ' +
            'Екатерины II;\n' +
            '2.	Документ, удостоверяющий личность и гражданство ' +
            'поступающего (копию);\n' +
            '3.	Документ об образовании установленного образца ' +
            '(оригинал или копию);\n' +
            '4.	СНИЛС (при наличии);\n' +
            '5.	Научный задел по каждой из выбранных конкурсных ' +
            'групп в печатном и электронном виде;\n' +
            '6.	Документы, подтверждающие дополнительные показатели ' +
            'поступающего к вступительному испытанию «Собеседование»;\n' +
            '7.	4 фотографии поступающего 3х4 см;\n' +
            '8.	Копия ИНН.'
        ),
        parse_mode="MARKDOWN"
    )
...
@save_user_and_messages
def dpk_asp(update, context):
    chat = update.effective_chat
    context.bot.send_message(
        chat_id=chat.id,
        text=(
            '*1.* Рекомендация для поступления в аспирантуру ' +
            'и статус «Кандидат для поступления в аспирантуру».' +
            'Для выпускников других университетов – Выписка из ' +
            'протокола ГЭК установленного образца с рекомендацией' +
            'для поступления в аспирантуру по научной специальности,' +
            'соответствующей выбранной конкурсной группе - 15 баллов;\n' +
            '*2.* Публикации в журналах, индексируемых в базах данных ' +
            'Web of Science, Scopus квартиля Q1-Q4 - До 7 баллов ' +
            '(независимо от количества);\n' +
            '*3.* Патент (на изобретение или полезную модель) ' +
            'или свидетельство (о государственной регистрации ' +
            'ЭВМ или базы данных) - До 5 баллов ' +
            '(независимо от количества);\n' +
            '*4.* Диплом о высшем образовании с отличием - 5 баллов;\n' +
            '*5.* Служба в вооруженных силах РФ по призыву, контракту,' +
            ' мобилизации - 5 баллов;\n' +
            '*6.* Публикации в журналах, рекомендованных ВАК, по научной ' +
            'специальности, соответствующей выбранной конкурсной ' +
            'группе - До 4 баллов (независимо от количества);\n' +
            '*7.* Диплом победителя, призера Международного ' +
            'форума-конкурса «Актуальные проблемы недропользования», ' +
            'диплом победителя, призера Всероссийской конференции ' +
            'студентов и аспирантов выпускного курса, за последние ' +
            '2 года - 4 балла (независимо от количества)\n' +
            '*8.* Диплом победителя, а также дипломы I, II, III ' +
            'степени Всероссийских и Международных научных ' +
            'конкурсов и конференций за последние 3 года - ' +
            '3 балла (независимо от количества);\n' +
            '*9.* Производственный опыт по научной специальности, ' +
            'соответствующей выбранной конкурсной группе - 2 балла.'
        ),
        parse_mode="MARKDOWN"
    )
...
@save_user_and_messages
def kcp_asp(update, context):
    chat = update.effective_chat
    context.bot.send_message(
        chat_id=chat.id,
        text=(
            'Количество мест для приема на обучение по ' +
            'образовательным программам высшего образования ' +
            '– программам подготовки научно-педагогических кадров ' +
            'в аспирантуре Санкт-Петербургского горного университета' +
            'императрицы Екатерины II в 2024 году можете посмотреть ' +
            '[ЗДЕСЬ](https://priem.spmi.ru/sites/default/files/manager' +
            '/08.aspirantura/2024/4.%20%D0%BA%D1%86%D0%BF.pdf)'
            ),
        parse_mode="MARKDOWN"
    )
...
@save_user_and_messages
def spiski_asp(update, context):
    chat = update.effective_chat
    context.bot.send_message(
        chat_id=chat.id,
        text=(
            'Список лиц, поступающих на обучение в аспирантуру: \n' +
            'https://priem2024.spmi.ru/aspirant/'
        ),
        parse_mode="MARKDOWN"
    )
...
@save_user_and_messages
def method_asp(update, context):
    chat = update.effective_chat
    context.bot.send_message(
        chat_id=chat.id,
        text=(
            'Абитуриент может подать документы следующими способами:\n' +
            '- в электронной форме посредством [электронной информационной ' +
            'системы Университета](http://tandem-online.spmi.ru/login);\n' +
            '- лично в Приемную комиссию ' +
            '(Набережная Лейтенанта Шмидта, 45)\n\n' +
            '[Памятка по заполнению заявления](http://priem-univer.ru/sites/' +
            'default/files/manager/08.aspirantura/pamyatka_asp.pdf)'
        ),
        parse_mode="MARKDOWN"
    )
...
@save_user_and_messages
def contact_asp(update, context):
    chat = update.effective_chat
    context.bot.send_message(
        chat_id=chat.id,
        text=(
...
dict = {
    ...
    r'Сроки подачи документов АСП':
        time_asp,
    r'Перечень вступительных испытаний АСП':
        vi_asp,
    r'Документы для поступления АСП':
        docs_asp,
    r'Списки поступающих АСП':
        spiski_asp,
    r'Конкурсные группы и количество мест АСП':
        kcp_asp,
    r'Способы подачи документов АСП':
        method_asp,
    r'Учет дополнительных компетенций АСП':
        dpk_asp,
    r'Контакты АСП':
        contact_asp,
    r'Списки поступающих':
        spiski,
    }
